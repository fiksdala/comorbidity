...
# Add map options for AHRQ Elixhauser options
map_choices <- c(
  names(.maps),
...
for (w in map_choices) {
  ...
  } else if (!grepl('elixhauser_ahrq_20', x = w)) {
  ...
  } else if (w == 'elixhauser_ahrq_2020_icd10_quan') {
    # Same computations as "elixhauser" (except carit removed)
    .weights[[w]][["vw"]] <- c(chf = 7, valv = -1, pcd = 4, pvd = 2, hypunc = 0, hypc = 0, para = 7, ond = 6, cpd = 3, diabunc = 0, diabc = 0, hypothy = 0, rf = 5, ld = 11, pud = 0, aids = 0, lymph = 9, metacanc = 12, solidtum = 4, rheumd = 0, coag = 3, obes = -4, wloss = 6, fed = 5, blane = -2, dane = -2, alcohol = 0, drug = -7, psycho = 0, depre = -3)
    # Swiss weights
    .weights[[w]][["swiss"]] <- c(chf = 13, valv = -1, pcd = 6, pvd = 3, hypunc = -4, hypc = -3, para = 11, ond = 10, cpd = 3, diabunc = 1, diabc = -1, hypothy = -3, rf = 8, ld = 16, pud = 0, aids = 0, lymph = 9, metacanc = 17, solidtum = 10, rheumd = -1, coag = 9, obes = -6, wloss = 6, fed = 5, blane = -5, dane = -7, alcohol = -3, drug = -5, psycho = -4, depre = -3)
  } else if (w == 'elixhauser_ahrq_2021') {
    # No weighted scoring options for elixhauser_ahrq_2021 currently available
  } else {
    # No weighted scoring options for elixhauser_ahrq_2021 currently available
  }
